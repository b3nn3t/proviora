...
export default function SupplementJar({ scrollProgress }: { scrollProgress: number }) {
  ...
  useFrame(() => {
    ...
    if (scrollProgress < 0.5) {
      ...
      jarRef.current.rotation.x = p * Math.PI * 0.4;
      jarRef.current.position.z = p * 2;
      jarRef.current.position.y = -p * 1;
    } else {
      const p = (scrollProgress - 0.5) * 2;
      jarRef.current.position.z = 2 + p * 10;
      jarRef.current.scale.setScalar(1 + p * 5);
    }
  });
    ...
    <>
      ...
      <group ref={jarRef}>
        <mesh position={[0, 0, 0]}>
          ...
          <meshStandardMaterial color="#001233" roughness={0.1} metalness={0.8} emissive="#001233" emissiveIntensity={0.2} />
        </mesh>
        ...
        <mesh position={[0, 0, 0.01]}>
          ...
          <meshStandardMaterial transparent opacity={0.1} color="#f5e6be" emissive="#f5e6be" emissiveIntensity={0.5} />
        </mesh>
      </group>
    ...
    <points ref={mesh}>
      <bufferGeometry>
        ...
        <bufferAttribute
          ...
          itemSize={3}
        />
      </bufferGeometry>      <pointsMaterial size={0.02} color="#d4af37" transparent opacity={0.6} sizeAttenuation />
    </points>
  );}
